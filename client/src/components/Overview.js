...
const Overview = ({ playerTag }) => {
  ...
  useEffect(() => {
    const fetchPlayerData = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/user/player-stats/${playerTag}`, {
          headers: {
            "x-auth-token": token,
          },
        });
  ...
  const generateBattleData = () => {
    ...
    // Last 10 battles data (randomly generated)
    const last10Battles = Array.from({ length: 10 }, (_, i) => {
      const result = Math.random() > 0.5 ? 'Win' : 'Loss';
      const trophiesChange = result === 'Win' ? Math.floor(Math.random() * 15) + 20 : -Math.floor(Math.random() * 15) - 20;
      return {
        id: i + 1, 
        result,
        trophiesChange,
    ...
    const trophyProgressionData = Array.from({ length: 6 }, (_, i) => {
      const base = playerData?.trophies ? playerData.trophies - (Math.floor(Math.random() * 300)) : 3000;
      return {
        name: getMonthName(i),
    ...
    return { pieData, last10Battles, trophyProgressionData, battlePerformanceData };
  };
  ...
  const { pieData, last10Battles, trophyProgressionData, battlePerformanceData } = playerData ? generateBattleData() : { pieData: [], last10Battles: [], trophyProgressionData: [], battlePerformanceData: [] };
    ...
    <Box sx={{ width: "100%" }}>
      ...
      >
        <Box sx={{ 
          ...
          {/* Player Avatar - Replaced with Pokeball-style avatar */}
          <Box 
            sx={{ 
              width: 80, 
              height: 80, 
          ...
          <Box sx={{
            ...
            <EmojiEventsIcon sx={{ color: pokemonStyles.colors.yellow, fontSize: 40, mb: 1 }} />
            <Typography variant="h4" sx={{ 
              fontWeight: "bold", 
              color: pokemonStyles.colors.blue,
              fontFamily: pokemonStyles.fonts.heading,
          ...
          <Card sx={{ 
            ...
            <CardContent sx={{ p: 3 }}>
              <Box sx={{ display: "flex", alignItems: "center", mb: 2 }}>
                <DiamondIcon sx={{ color: pokemonStyles.colors.yellow, mr: 1.5 }} />
            ...
            <Box sx={{ height: 250, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip formatter={(value, name) => [`${(value / (wins + losses) * 100).toFixed(0)}%`, name]} />
                  <Legend />
                </PieChart>
              </ResponsiveContainer>
            </Box>
            ...
            <Box sx={{ height: 250, width: '100%' }}>
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Tooltip formatter={(value) => [`${value} Trophies`, 'Trophies']} />
                  <Legend />
                  <Line 
                    type="monotone" 
                    dataKey="trophies" 
        ...
        <Grid item xs={12} md={4}>
          ...
          >
            ...
            <Box sx={{ height: 250, width: '100%', display: 'flex', justifyContent: 'center' }}>
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart cx="50%" cy="50%" outerRadius="65%" data={battlePerformanceData}>
            ...
            <Grid item xs={6} sm={3} md={1.5} key={index}>
              <Box 
                sx={{ 
                  ...
                  p: 1.5,
                  borderRadius: 1,
                  bgcolor: chest.rarity === 'Legendary' ? 'rgba(255, 215, 0, 0.1)' : 
                          chest.rarity === 'Epic' ? 'rgba(156, 39, 176, 0.1)' : 
                          chest.rarity === 'Rare' ? 'rgba(59, 76, 202, 0.1)' : 'rgba(158, 158, 158, 0.1)',
                  border: chest.rarity === 'Legendary' ? '1px solid #FFD700' : 
                          chest.rarity === 'Epic' ? '1px solid #9C27B0' : 
                          chest.rarity === 'Rare' ? '1px solid #3B4CCA' : '1px solid #9E9E9E',
                }}
              >
                ...
                {chest.rarity === 'Legendary' ? (
                  ...
                  <LockOpenIcon sx={{ color: '#3B4CCA', fontSize: 32, my: 1 }} />
                ) : (
                  <LockIcon sx={{ color: '#9E9E9E', fontSize: 32, my: 1 }} />
                )}
                
                <Typography variant="body2" sx={{ fontWeight: 'bold', textAlign: 'center', fontSize: '0.75rem' }}>
                  {chest.type}
                </Typography>
              </Box>
            </Grid>
      ...
      >
        ...
        <TableContainer>
          <Table size="small">
            ...
            <TableBody>
              {last10Battles.map((battle) => (
                <TableRow key={battle.id} sx={{ 
                  ...
                  <TableCell sx={{ 
                    color: battle.result === 'Win' ? '#4CAF50' : pokemonStyles.colors.red,
                    fontWeight: 'bold'
                  }}>
                    {battle.result}
                  </TableCell>
                  <TableCell 
                    align="right" 
                    sx={{ 
                      color: battle.trophiesChange > 0 ? '#4CAF50' : pokemonStyles.colors.red,
                      fontWeight: 'bold'
                    }}
                  >
                    {battle.trophiesChange > 0 ? `+${battle.trophiesChange}` : battle.trophiesChange}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
        ...
        {currentFavouriteCard && (
          <Box sx={{ mt: 4 }}>
            ...
            <Card sx={{ 
              ...
              color: '#333',
              borderRadius: 2,
              border: `1px solid ${pokemonStyles.colors.blue}`,
              boxShadow: '0 4px 12px rgba(59, 76, 202, 0.05)'
            }}>
              <Box 
                sx={{ 
                  width: 60, 
                  ...
                  borderRadius: '50%',
                  position: 'relative',
                  border: `2px solid ${pokemonStyles.colors.blue}`,
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                  bgcolor: 'rgba(59, 76, 202, 0.1)'
              ...
              >
                <DiamondIcon sx={{ fontSize: 30, color: pokemonStyles.colors.blue }} />
              </Box>
              <Stack>
                <Typography variant="h6" sx={{ 
                  fontWeight: 'bold',
                ...
                <Typography variant="body2" sx={{
                  ...
                  Level {currentFavouriteCard.level || "?"}
                </Typography>
              </Stack>
            </Card>
          </Box>
